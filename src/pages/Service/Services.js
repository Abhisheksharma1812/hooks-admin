...
import { Button, Pagination,Row,Col, Table, Form } from "react-bootstrap";
    ...
    alertify.confirm(
      ...
      async function () {
        try {
          ...
          alertify.error(error.response?.data?.message || "Failed to delete service.");
        }
      },
      function () {
          ...
          {currentServices.length > 0 ? (
            currentServices.map((service, index) => (
              <tr key={index}>
                ...
                <td>
                  <Link href="#" className="me-2" onClick={() => handleDeleteService(service._id)}>
                    <FaRegTrashAlt />
                  </Link>
