    ...
    alertify.confirm(
      ...
      async function () {
        try {
          ...
          setFilteredEvents((prev) => prev.filter((service) => service._id !== id));
          alertify.success("Event deleted successfully!");
        } catch (error) {
          console.error("Error deleting user:", error);
          alertify.error(error.response?.data?.message || "Failed to delete event.");
        }
      },
      function () {
          ...
          {currentEvents.length > 0 ? (
            currentEvents.map((event, index) => (
              <tr key={index}>
                ...
                <td>{event?.zipcode || ""}</td>
                <td>
            <Link href="#" className="me-2" onClick={() => handleDeleteEvent(event._id)}>
              <FaRegTrashAlt />
            </Link>
