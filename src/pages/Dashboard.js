...
const DashboardContent = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const userResponse = await axios.get(`${process.env.REACT_APP_API_URL}/user/all`, {
          headers: {
        ...
        const eventsResponse = await axios.get(`${process.env.REACT_APP_API_URL}/event/all`, {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("Auth-Token")}`,
          },
        });
